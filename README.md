...
Windows** y abre la GUI una sola vez. Flags: `--interval N`, `--no-launch`,
`--no-notify`.

Para probarlo sin reiniciar, **doble clic en `vigilar.bat`** (o
`python unlooktool_watch.py` en una terminal).

> ⚠️ Haz doble clic en los `.bat`, **no** en los `.py`: en Windows un `.py`
> suele abrirse en el editor en vez de ejecutarse. Los `.bat` sí lo ejecutan.
